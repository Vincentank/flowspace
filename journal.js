    ...
    <div class="entry-layout">
      ...
      <div class="panel entry-panel">
        ...
        <div class="tag-input-wrap">
          <div class="tag-chips" id="tag-chips-${dateStr}">
            ${entry.tags.map(t => `
              <span class="tag-chip">
                ${t}
                <button class="tag-chip-remove" onclick="removeTag('${dateStr}', '${t}')">×</button>
              </span>
            `).join('')}
          </div>
          <input class="tag-input" id="tag-input-${dateStr}"
            type="text" placeholder="Add a tag and press Enter (e.g. chill, travel, ideas...)"
            onkeydown="handleTagInput(event, '${dateStr}')"/>
        </div>
      </div>
      ...
      <div class="panel entry-panel">
        ...
        <div id="journal-gphotos">${renderJournalPhotos(dateStr)}</div>
      </div>

      <!-- GOOGLE CALENDAR EVENTS -->
      <div class="panel entry-panel">
        <div class="entry-section-title">Google Calendar</div>
        <div id="journal-gcal-events">${renderJournalEvents(dateStr)}</div>
      </div>
...
// trigger async load of google data for this date
function afterJournalEntryRender(dateStr) {
  loadJournalGoogleData(dateStr);
}
...
function openJournalEntry(dateStr) {
  ...
  getEntry(dateStr);
  renderPage('journal');
  setTimeout(() => afterJournalEntryRender(dateStr), 100);
}
...
function handleTagInput(e, dateStr) {
  if (e.key !== 'Enter' && e.key !== ',') return;
  e.preventDefault();
  const input = document.getElementById('tag-input-' + dateStr);
  ...
  if (!entry.tags.includes(val)) {
    entry.tags.push(val);
    saveJournal();
    const chips = document.getElementById('tag-chips-' + dateStr);
    if (chips) {
      const chip = document.createElement('span');
      chip.className = 'tag-chip';
      chip.innerHTML = `${val}<button class="tag-chip-remove" onclick="removeTag('${dateStr}', '${val}')">×</button>`;
...
function removeTag(dateStr, tag) {
  const entry = getEntry(dateStr);
  ...
  if (chips) {
    chips.querySelectorAll('.tag-chip').forEach(el => {
      if (el.textContent.trim().startsWith(tag)) el.remove();
    });
  }
}
